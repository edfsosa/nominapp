document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadModels() {
        try {
            ...
            logStatus('Error al cargar los modelos. Por favor, recarga la página.');
    ...
    async function captureDescriptor(samples = 5, intervalMs = 160) {
        ...
        try {
            while (list.length < samples) {
                ...
                logStatus(`Capturando rostro (${list.length + 1} de ${samples})…`);
                ...
                if (++attempts > samples * 3) {
                    throw new Error("No se pudieron capturar suficientes muestras de rostro.");
                }
    ...
    btnIdentify.addEventListener('click', async () => {
        ...
        try {
            ...
            if (!resp.ok) {
                throw new Error(`Error de red: ${resp.status} ${resp.statusText}`);
            }
